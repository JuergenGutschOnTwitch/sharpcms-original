...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template mode="edit" match="folder">
			...
			<div class="adminmenu">
				...
				<a class="button">
					<xsl:attribute name="href">javascript:ThrowEventNew('renamefolder','<xsl:value-of select="@path"/>','Write the new name')</xsl:attribute>			
					Rename folder
				</a> 
				<a class="button">
					<xsl:attribute name="href">javascript:ThrowEventNew('addfolder','<xsl:value-of select="@path"/>','Type the name of the new folder')</xsl:attribute>			
					Add subfolder
				</a>
				<a class="button">
					<xsl:attribute name="href">javascript:ModalDialogShow('<xsl:value-of select="/data/basepath"/>/default.aspx?process=admin/choose/folder','ReturnMethodMoveFolder()');
        </xsl:attribute>
					Move folder
				</a>
	...
	<xsl:template name="upload">
		<div class="adminmenu">
			Upload new file in directory<br/>
